...
<script>
    document.getElementById("createCustomerForm").addEventListener("submit", function (event) {
        ...
        if (fullName.value.trim() === "") {
        ...
        } else if (cccd.value.trim() !== "" && !/^\d{12}$/.test(cccd.value.trim())) {
            errorMsg = "CCCD must be exactly 12 digits if provided.";
            ...
            if (age < 18) {
                errorMsg = "Customer must be at least 18 years old.";
            }
        }
        if (password.value.length < 8 || !/[A-Z]/.test(password.value) || !/[!@#$%^&*_-]/.test(password.value)) {
