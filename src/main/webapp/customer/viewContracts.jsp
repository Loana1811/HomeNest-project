    ...
    <body>
        <div class="container">
            <div class="contract-container">
                ...
                <c:choose>
                    <c:when test="${not empty highlightContractId and not empty contracts}">
                        <!-- Display the specific contract based on highlightContractId -->
                        <c:forEach var="contract" items="${contracts}">
                            <c:if test="${contract.contractId == highlightContractId}">
...
<li>Hợp đồng này có hiệu lực từ ngày <fmt:formatDate value="${contract.contractCreatedAt}" pattern="dd/MM/yyyy" /> hoặc ${contract.contractCreatedAt == null ? 'Chưa xác định' : ''}.</li>
                                ...
                                <div class="text-center mt-4">
                                    ...
                                    <c:if test="${not empty contract.contractId and contract.contractStatus != 'Active' and contract.contractStatus != 'Terminated'}">
                                        <a href="${ctx}/HomeNest/Contracts?action=accept&id=${contract.contractId}" class="btn btn-success ms-2">Chấp Nhận</a>
                                        <a href="${ctx}/HomeNest/Contracts?action=reject&id=${contract.contractId}" class="btn btn-danger ms-2">Từ Chối</a>
                                    </c:if>
                                    <a href="contracts/generatePdf?id=${contract.contractId}" class="btn btn-teal ms-2" target="_blank">Tải PDF</a>
<a href="<%=ctx%>/customer/room-" class="btn btn-secondary ms-2">Quay Lại</a>
                                </div>
                            </c:if>
                        </c:forEach>
                    </c:when>
