...
<div class="edit-container">
    ...
    <c:if test="${user.role != null && user.role.roleName eq 'Admin'}">
        <div class="alert alert-info">
            Admin accounts cannot be edited. (Read-only)
        </div>
    </c:if>
    ...
    <form 
        ...
        method="post" 
        novalidate
        <c:if test="${user.role != null && user.role.roleName eq 'Admin'}">onsubmit="return false;"</c:if>
    >
        <input type="hidden" name="action" value="edit"/>
        <input type="hidden" name="userID" value="${user.userID}"/>
        <input type="hidden" name="roleID" value="${user.role != null ? user.role.roleID : ''}"/>
        <input type="hidden" id="originalStatus" value="${user.userStatus}"/>

        <!-- Full Name -->
        <div class="form-section">
            <div class="form-floating mb-3">
                <input type="text" id="userFullName" name="fullName" class="form-control"
                       ...
                       <c:if test="${user.role != null && user.role.roleName eq 'Admin'}">readonly</c:if>
                       required />
        ...
        <div class="form-section">
            <div class="form-floating mb-3">
                <input type="text" class="form-control" id="role" name="role"
                       value="${user.role != null ? user.role.roleName : (user.roleID == 1 ? 'Admin' : user.roleID == 2 ? 'Manager' : 'Manager')}"
                       readonly style="background-color:#eaf4fa;" />
                <label for="role">Role</label>
            </div>
        </div>

        <!-- Block (for Manager only) -->
        <c:if test="${user.role != null && user.role.roleName eq 'Manager' && not empty blockList}">
            <div class="form-section">
                <div class="form-floating mb-3">
        ...
        <div class="form-section">
            <div class="form-floating mb-3">
                <select id="status" name="status" class="form-select"
                        <c:if test="${user.role != null && user.role.roleName eq 'Admin'}">disabled</c:if>>
                    <option value="Active" <c:if test="${param.status eq 'Active' or requestScope.formStatus eq 'Active' or user.userStatus eq 'Active'}">selected</c:if>>Active</option>
                    <option value="Disable" <c:if test="${param.status eq 'Disable' or requestScope.formStatus eq 'Disable' or user.userStatus eq 'Disable'}">selected</c:if>>Inactive</option>
                </select>
