...
public class NotificationDAO extends DBContext {
        ...
        try ( Connection conn = dbContext.getConnection();  PreparedStatement pstmt = conn.prepareStatement(sql)) {
            ...
            try ( ResultSet rs = pstmt.executeQuery()) {
                if (rs.next()) {
                    ...
                    n.setRead(rs.getBoolean("IsRead")); // Sử dụng setter
                    n.setNotificationCreatedAt(rs.getTimestamp("NotificationCreatedAt"));
                    n.setSentBy(rs.getObject("SentBy", Integer.class));
                    return n;
                }
            }
        }
    ...
    public List<Notification> getNotificationsWithSenderNameByCustomer(int customerId) {
        ...
        try ( Connection conn = getConnection();  PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            while (rs.next()) {
                ...
                n.setTitle(rs.getString("Title"));
                n.setMessage(rs.getString("Message"));
                n.setRead(rs.getBoolean("IsRead")); // Đã đúng
                n.setNotificationCreatedAt(rs.getTimestamp("NotificationCreatedAt"));
    ...
    public void insert(Notification n) {
        ...
        try ( Connection conn = getConnection();  PreparedStatement ps = conn.prepareStatement(sql)) {
            ...
            int rows = ps.executeUpdate();
            System.out.println("✅ Notification inserted for CustomerID = " + n.getCustomerID() + " | Rows affected = " + rows);
        } catch (SQLException e) {
    ...
    public List<Notification> getByCustomerId(int customerId) throws SQLException {
        List<Notification> list = new ArrayList<>();
        String sql = "SELECT n.*, u.FullName AS SenderName "
                + "FROM Notifications n LEFT JOIN Users u ON n.SentBy = u.UserID "
                + "WHERE n.CustomerID = ? ORDER BY n.NotificationCreatedAt DESC";
        try ( Connection conn = getConnection();  PreparedStatement ps = conn.prepareStatement(sql)) {
            ps.setInt(1, customerId);
            ResultSet rs = ps.executeQuery();
            while (rs.next()) {
