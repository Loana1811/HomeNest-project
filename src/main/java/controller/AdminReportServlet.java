        ...
        try {
            ...
            if ("resolve".equals(action)) {
                try {
                    ...
                    ReportDAO reportDAO = new ReportDAO(); // Tạo instance mới để gọi getReportById
                    ...
                    Report currentReport = reportDAO.getReportById(reportID); // Sử dụng instance hiện tại
                    if (currentReport != null && "Closed".equals(currentReport.getReportStatus())) {
                        session.setAttribute("error", "This report is already closed and cannot be modified.");
                        response.sendRedirect("adminReport");
                        return;
                    }
                    ...
                    if (success) {
                        if ("Closed".equals(status)) {
                            session.setAttribute("message", "Report closed and saved successfully!");
                        } else {
