...
//            } else {
//                room = roomDAO.getAllRoomIdName(); // fallback nếu chưa chọn block
//            }
//
...
//                    && readingMonth != null && !readingMonth.isEmpty()) {
//                int roomId = Integer.parseInt(roomIdStr);
//                int typeId = Integer.parseInt(typeIdStr);
//                double oldIndex = dao.getLatestIndex(roomId, typeId, readingMonth);
...
protected void doPost(HttpServletRequest req, HttpServletResponse resp)
    ...
    try {
        String roomIdStr = req.getParameter("roomId");
        String readingMonth = req.getParameter("readingMonth");
        String blockId = req.getParameter("blockId"); // lấy block để redirect lại
        ...
        for (String typeIdStr : typeIds) {
            ...
            if (newIndexStr == null || newIndexStr.isEmpty()) continue;

            BigDecimal newIndex = new BigDecimal(newIndexStr);
            double oldVal = dao.getLatestIndex(roomId, typeId, readingMonth);
        ...
        // ✅ Sau khi ghi xong thì redirect về bước 1 (danh sách phòng theo block)
