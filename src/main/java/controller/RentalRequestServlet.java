...
protected void doPost(HttpServletRequest request, HttpServletResponse response)
    ...
    try {
        ...
        // ✅ Cập nhật các yêu cầu quá 7 ngày thành Expired
        requestDAO.expireOldPendingRequests();

        // ✅ Kiểm tra xem đã gửi yêu cầu cho phòng này chưa
        ...
        // ✅ Gửi thông báo đến các manager
        RoomDAO roomDAO = new RoomDAO();
        Room room = roomDAO.getRoomById(roomId);
        int blockId = room.getBlockID();

        UserDAO userDAO = new UserDAO();
        List<User> managers = userDAO.getManagersByBlockId(blockId);
        ...
        if (managers != null && !managers.isEmpty()) {
            ...
            for (User manager : managers) {
                if (!notiDAO.existsSimilarNotification(manager.getUserId(), title, message)) {
                    ...
                    notification.setNotificationCreateAt(new Timestamp(System.currentTimeMillis()));
                    notification.setSentBy(currentCustomer.getCustomerFullName());
